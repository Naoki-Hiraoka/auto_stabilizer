...
public:
  ...
  double touchVel = 0.3; // 0より大きい. 単位[m/s]. この速さで足を下ろした場合に着地までに要する時間をremainTimeが下回るまで、足下げを始めない. 0.5だと実機では少し速すぎるか? maxSwingVel[2]以下
...
protected:
  ...
  double fcp(double t, double omega, double cp, double zmp) const {return std::exp(omega * t) * (cp - zmp) + zmp;};
  double fsw(double t, double tmin, double vmax, double sw) const {return (t - tmin) * vmax + sw;};
  double dfcp(double t, double omega, double cp, double zmp) const {return omega * std::exp(omega * t) * (cp - zmp);};
  double dfsw(double t, double vmax) const {return vmax;};
  double calc_extTime(double omega, double cp, double zmp, double vmax) const {return std::log(vmax/(omega*(cp - zmp))) / omega;};
  double calcCRMinMaxTime(double& minTime, double& maxTime, double delta, double omega, double cp, double zmp, double tmin, double vmax, double sw) const;
